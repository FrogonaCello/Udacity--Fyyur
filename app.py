...
class Venue(db.Model):
    ...
    # TODO: implement any missing fields, as a database migration using Flask-Migrate // Migrate done, missing fields still missing: maybe comment, tags
    
    def __init__(self, name, genres, city, state, address, phone, image_link, website, facebook_link, seeking_talent=False, seeking_description=""):
        self.name = name
        self.genres = genres
        self.city = city
        self.state = state
...
def venues():
  ...
  #       num_shows should be aggregated based on number of upcoming shows per venue.
    ...
    areas = Venue.query.group_by(Venue.id, Venue.state, Venue.city).all()
...
def show_artist(artist_id):
    ...
    past_shows = []
    upcoming_shows = db.session.query(Show).filter(Show.venue_id==1).filter(Show.start_time>datetime.now()).all()
    upcoming_shows = []
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        #db.session.close()
        if error:
            flash('Sorry. Artist could not be changed.')
            db.session.rollback()
        if not error:
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        db.session.close()
    if error:
        flash(f'Sorry. Venue could not be changed.')
    if not error:
...
def create_artist_submission():
    ...
    try:
        ...
        if not error:
            ...
            flash('Sorry. Artist ' + data['name'] + ' could not be listed.')
            db.session.rollback() 
...
def create_show_submission():
    ...
    try:
        ...
        if not error:
            db.session.commit()
            flash('Show on ' + data['start_time'] + ' was successfully listed!')
        if error:
